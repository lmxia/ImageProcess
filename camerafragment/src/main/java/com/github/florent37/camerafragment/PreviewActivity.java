...
public class PreviewActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private static final String REQUEST_URL = "http://18.191.145.203:8000/image/process";
    ...
    public void onClick(View view) {
        ...
        String mimeType = MimeTypeMap.getSingleton()
                .getMimeTypeFromExtension(
                        MimeTypeMap.getFileExtensionFromUrl(previewFilePath));
        if (view.getId() == R.id.confirm_media_result) {
            new UploadFileTask(this).setAsynUpdateListener(new AsynUpdateListener() {
                @Override
                public void onDataReceivedSuccess(List<List<String>> listData) {
                    StringBuffer stringBuffer = new StringBuffer();
                    int index = 1;
                    for (List<String>  itemInfo :listData ){
                        stringBuffer.append("TOP " + index++ + "   " + itemInfo.get(0) + "\r\n");
                    }
                    imageInfo.setText(stringBuffer.toString());
                    imageInfo.setTextColor(Color.RED);
                }
            ...
            }).execute(previewFilePath, mimeType,
